...
function loadPage(page){
	...
	$.ajax({
		url : "list.jsp?page=" + page + "&pageRows=" + pageRows
		, type : "GET"
		, cache : false
...
//			result += "<td><input type='checkbox' name='uid' value='" + items[i].uid + "'></td>\n";
			...
			result += "<td>${dto.uid }</td>\n";
			result += "<td><a href=\"view.do?uid=${dto.uid }\">${dto.subject }</a></td>\n";
			result += "<td>${dto.name }</td>\n";
			result += "<td>${dto.viewCnt }</td>\n";
			result += "<td>${dto.regDate }</td>\n";	
...
function addViewEvent(){
	...
	$("#list .subject").click(function(){
		...
		// 읽어오기
		$.ajax({
			url : "view.jsp?uid=" + $(this).attr('data-uid'),
			type : "GET",
			cache : false,
			success : function(data, status){
				if(status == "success"){
